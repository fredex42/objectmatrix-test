...
class OMFastContentSearchSource(userInfo:UserInfo, contentSearchString:String, atOnce:Int=100) extends GraphStage[SourceShape[ObjectMatrixEntry]] {
  ...
  override def createLogic(inheritedAttributes: Attributes): GraphStageLogic = new GraphStageLogic(shape) {
    ...
    def parseOutResults(resultString:String) = {
      ...
      ObjectMatrixEntry(parts.head,attributes = Some(mxsMeta), fileAttribues = None, vault = vault.get)
    }
    ...
    override def preStart(): Unit = {
      ...
      try {
        ...
        //iterator = vault.map(_.searchObjectsIterator(searchTerms.head, atOnce).asScala)
        iterator = vault.map(_.searchObjectsIterator(SearchTerm.createSimpleTerm(Constants.CONTENT, contentSearchString), atOnce).asScala)
